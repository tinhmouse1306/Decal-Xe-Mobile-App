...
import com.example.decalxeandroid.data.repository.OrderDetailRepositoryImpl
import com.example.decalxeandroid.data.repository.OrderRepositoryImpl
import com.example.decalxeandroid.data.repository.OrderStageHistoryRepositoryImpl
...
object AppContainer {
    ...
    // API Services
    private val customerApi: CustomerApi by lazy { retrofit.create(CustomerApi::class.java) }

    private val customerVehicleApi: CustomerVehicleApi by lazy {
        retrofit.create(CustomerVehicleApi::class.java)
    }

    private val employeeApi: EmployeeApi by lazy { retrofit.create(EmployeeApi::class.java) }
    ...
    private val orderDetailApi: OrderDetailApi by lazy {
        retrofit.create(OrderDetailApi::class.java)
    }

    private val orderStageHistoryApi: OrderStageHistoryApi by lazy {
        retrofit.create(OrderStageHistoryApi::class.java)
    }

    private val tokenManager: TokenManager by lazy { TokenManager() }

    // Mappers
    private val customerMapper: CustomerMapper by lazy { CustomerMapper() }

    private val customerVehicleMapper: CustomerVehicleMapper by lazy { CustomerVehicleMapper() }

    private val employeeMapper: EmployeeMapper by lazy { EmployeeMapper() }

    private val orderMapper: OrderMapper by lazy { OrderMapper() }

    private val orderDetailMapper: OrderDetailMapper by lazy { OrderDetailMapper }

    private val orderStageHistoryMapper: OrderStageHistoryMapper by lazy { OrderStageHistoryMapper }

    private val decalServiceMapper: DecalServiceMapper by lazy { DecalServiceMapper() }
    ...
    val orderDetailRepository: OrderDetailRepository by lazy {
        OrderDetailRepositoryImpl(orderDetailApi)
    }

    val orderStageHistoryRepository: OrderStageHistoryRepository by lazy {
        OrderStageHistoryRepositoryImpl(orderStageHistoryApi)
    }
    ...
    val vehicleRepository: VehicleRepository by lazy { VehicleRepositoryImpl(vehicleApiService) }
}

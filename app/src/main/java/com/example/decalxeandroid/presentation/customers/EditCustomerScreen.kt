...
fun EditCustomerScreen(
    ...
    viewModel: EditCustomerViewModel = androidx.lifecycle.viewmodel.compose.viewModel(
        factory = EditCustomerViewModelFactory(
    ...
    LaunchedEffect(customerId) {
        viewModel.loadAndInitializeCustomer(customerId)
    }
            ...
            when (uiState) {
                is EditCustomerUiState.Editing -> {
                    ...
                    ModernWelcomeCard(
                        ...
                        subtitle = "Cập nhật thông tin khách hàng ${editingState.formData.firstName} ${editingState.formData.lastName}"
                    )
                }
                else -> {
                    ModernWelcomeCard(
...
class EditCustomerViewModel(
    private val customerRepository: com.example.decalxeandroid.domain.repository.CustomerRepository
) : androidx.lifecycle.ViewModel() {
    
    private val _uiState = androidx.compose.runtime.mutableStateOf<EditCustomerUiState>(
                    ...
                    when (result) {
                        is com.example.decalxeandroid.domain.model.Result.Success -> {
                            ...
                            val formData = CustomerFormData(
                                ...
                                lastName = customer.fullName.split(" ").drop(1).joinToString(" "),
                        ...
                        is com.example.decalxeandroid.domain.model.Result.Error -> {
                            _uiState.value = EditCustomerUiState.Error(
                                result.message ?: "Lỗi không thể tải thông tin khách hàng"
                            )
                        }
    ...
    fun updateFirstName(firstName: String) {
        val currentFormData = (uiState.value as? EditCustomerUiState.Editing)?.formData ?: CustomerFormData()

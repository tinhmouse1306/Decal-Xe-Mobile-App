...
fun EditCustomerVehicleScreen(
    ...
    viewModel: EditCustomerVehicleViewModel = androidx.lifecycle.viewmodel.compose.viewModel(
        factory = EditCustomerVehicleViewModelFactory(
            customerVehicleRepository = com.example.decalxeandroid.di.AppContainer.customerVehicleRepository,
            vehicleRepository = com.example.decalxeandroid.di.AppContainer.vehicleRepository
        )
    )
) {
    ...
    LaunchedEffect(vehicleId) {
        viewModel.loadAndInitializeVehicle(vehicleId)
    }
    ...
    Card(
        ...
        Column(
            ...
            VietnameseDropdownField(
                ...
                options = listOf(
                    "Đen", "Trắng", "Xám", "Bạc", "Đỏ", "Xanh dương", "Xanh lá", "Vàng", "Cam", "Tím", "Nâu"
                ),
...
class EditCustomerVehicleViewModel(
    private val customerVehicleRepository: com.example.decalxeandroid.domain.repository.CustomerVehicleRepository,
    private val vehicleRepository: com.example.decalxeandroid.domain.repository.VehicleRepository
) : androidx.lifecycle.ViewModel() {
    
    private val _uiState = androidx.compose.runtime.mutableStateOf<EditCustomerVehicleUiState>(
    ...
    private val _vehicleModels = androidx.compose.runtime.mutableStateOf<List<VehicleModelDto>>(emptyList())
    ...
    fun loadAndInitializeVehicle(vehicleId: String) {
        viewModelScope.launch {
            ...
            try {
                customerVehicleRepository.getVehicleById(vehicleId).collect { result ->
                    when (result) {
                        ...
                        is com.example.decalxeandroid.domain.model.Result.Error -> {
                            _uiState.value = EditCustomerVehicleUiState.Error(
                ...
                _uiState.value = EditCustomerVehicleUiState.Error("Lỗi không xác định: ${e.message}")
            }
        }
    }
            ...
            try {
                vehicleRepository.getVehicleModels().collect { result ->
                    when (result) {
                        ...
                        is com.example.decalxeandroid.domain.model.Result.Error -> {
                            android.util.Log.e("EditCustomerVehicleVM", "Failed to load vehicle models: ${result.message}")
